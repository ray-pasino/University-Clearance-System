...
const Page = () => {
  ...
  const [hodData, setHodData] = useState({ total: 0, approved: 0 });
  ...
  useEffect(() => {
    ...
    const fetchHodData = async () => {
      try {
        ...
        const totalHod = totalRes.data.departmentCounts.find(
          (dept: DepartmentCount) => dept.department === "Head of Departments"
        )?.count || 0;

        const approvedHod = approvedRes.data.approvedDepartmentCounts.find(
          (dept: ApprovedDepartmentCount) => dept.department === "Head of Departments"
        )?.approvedCount || 0;

        setHodData({ total: totalHod, approved: approvedHod });
      } catch (err) {
  ...
  const handleAction = async (
    ...
    try {
      const res = await axios.post(
        ...
        {
          requestId,
          department: "Head of Departments", // Ensure this matches the department in your data
        },
        {
          headers: {
          ...
          req._id === requestId
            ? {
                ...
                departments: req.departments.map((dept: any) =>
                  dept.name === "Head of Departments" ? { ...dept, status: decision } : dept
                ),
              }
            : req
        )
  ...
  return (
    <div>
      ...
      {/* Head of Departments Department Stats */}
      <h2 className="text-lg font-semibold text-gray-700 mx-10 p-4 py-2">
        Head of Departments Department Statistics
      </h2>
      <div className="mx-10 p-4 py-6 sm:py-10 mb-6 text-gray-400 flex flex-col sm:flex-row space-y-6 sm:space-y-0 sm:space-x-8 items-center">
        <div className="p-4 sm:p-8 w-full flex flex-col items-center bg-white border-t-4 border-t-red-300 rounded-2xl shadow-md transition-transform transform hover:scale-105">
          ...
          <h3 className="text-2xl font-bold text-red-500">{hodData.total}</h3>
        ...
        <div className="p-4 sm:p-8 w-full flex flex-col items-center bg-white border-t-4 border-t-blue-300 rounded-2xl shadow-md transition-transform transform hover:scale-105">
          ...
          <h3 className="text-2xl font-bold text-blue-500">{hodData.approved}</h3>
      ...
      <h2 className="pl-12 sm:mb-4 md:text-lg text-gray-700 font-semibold">
        Head of Departments Clearance Requests
      </h2>
      ...
      <div className="mx-10 p-4 py-6 sm:py-10 mb-6 bg-white text-gray-400 mt-6 rounded-lg text-[10px] sm:text-[13px] shadow-md">
        {requests.length === 0 ? (
          ...
          requests
            .filter((req) =>
              req.departments.some((dept: any) => dept.name === "Head of Departments")
            )
            .map((req) => {
              const hodDept = req.departments.find(
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-4">
                    ...
                    <span
                      className={`inline-block px-3 py-1 rounded-full font-semibold text-xs ${
                        hodDept?.status === "Requested"
                          ? "bg-blue-100 text-blue-700"
                          : hodDept?.status === "Pending"
                          ? "bg-yellow-100 text-yellow-700"
                          : hodDept?.status === "Approved"
          ...
          >
            ...
            <table className="min-w-full divide-y divide-gray-200 rounded-md overflow-hidden text-sm text-left mt-10">
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                <tr>
                  <td className="py-3 px-2 text-[12px]">Introduction to Computer Science</td>
                  <td className="py-3 px-2 text-[12px]">A</td>
                  <td className="py-3 px-2 text-[12px]">3.75</td>
                </tr>
                <tr className="bg-gray-50">
                  <td colSpan={2} className="py-3 px-2 font-bold text-gray-600">
                    Cumulative GPA:
                  </td>
                  <td className="py-3 px-2 font-bold text-indigo-400">3.75</td>
                </tr>
              </tbody>
            </table>
