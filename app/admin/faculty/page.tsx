...
const Page = () => {
  ...
  const [facultyData, setFacultyData] = useState({ total: 0, approved: 0 });
  ...
  useEffect(() => {
    ...
    const fetchFacultyData = async () => {
      try {
        ...
        const totalFaculty = totalRes.data.departmentCounts.find(
          (dept: DepartmentCount) => dept.department === "Faculty"
        )?.count || 0;

        const approvedFaculty = approvedRes.data.approvedDepartmentCounts.find(
          (dept: ApprovedDepartmentCount) => dept.department === "Faculty"
        )?.approvedCount || 0;

        setFacultyData({ total: totalFaculty, approved: approvedFaculty });
      } catch (err) {
  ...
  const handleAction = async (
    ...
    try {
      const res = await axios.post(
        ...
        {
          requestId,
          department: "Faculty", // Ensure this matches the department in your data
        },
        {
          headers: {
    ...
    <div>
      ...
      <div className="mx-10 p-4 py-6 sm:py-10 mb-6 text-gray-400 flex flex-col sm:flex-row space-y-6 sm:space-y-0 sm:space-x-8 items-center">
        <div className="p-4 sm:p-8 w-full flex flex-col items-center bg-white border-t-4 border-t-red-300 rounded-2xl shadow-md transition-transform transform hover:scale-105">
          ...
          <h3 className="text-2xl font-bold text-red-500">{facultyData.total}</h3>
        ...
        <div className="p-4 sm:p-8 w-full flex flex-col items-center bg-white border-t-4 border-t-blue-300 rounded-2xl shadow-md transition-transform transform hover:scale-105">
          ...
          <h3 className="text-2xl font-bold text-blue-500">{facultyData.approved}</h3>
