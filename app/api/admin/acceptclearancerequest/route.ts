...
export async function POST(request: Request) {
    ...
    try {
        ...
        // Check if all departments except "Head of Alumni Relations" are approved
        const allApprovedExceptAlumni = clearanceRequest.departments
            .filter((d: any) => d.name !== "Head of Alumni Relations")
            .every((d: any) => d.status === "Approved");

        clearanceRequest.overallStatus = allApprovedExceptAlumni ? "Approved" : "Pending";
        clearanceRequest.updatedAt = new Date();

        await clearanceRequest.save();

        // If all except "Head of Alumni Relations" are approved, update the student's clearance status
        if (allApprovedExceptAlumni) {
